...
std::string message::formatted_message(rightway::system::error_code & ec)
{
    time_t rawtime;
    tm * timeinfo;
    char formatted_date[80];

    ec = rightway::system::success;

    ::time(&rawtime);

#ifdef _MSC_VER
	tm timeinfo_s;
	errno_t err = ::localtime_s(&timeinfo_s, &rawtime);

    if (err)
    {
        ec = rightway::smtp::error::invalid_argument;
        return "";
    }
